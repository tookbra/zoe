...
var Affix = {
  init: function(element, options) {
    ...
    window.matchMedia('(min-width: 992px)').addListener(event => {
      if (event.matches) {
        this.offset = Zoe.utils.getAffixParam();
        this.checkPosition();
      }
    });
  },
...
Zoe.utils.getAffixParam = function() {
  const sidebarOffset = CONFIG.sidebar.offset || 12;

  let headerOffset = document.querySelector('.pd-header').offsetHeight + sidebarOffset;
  return {
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  Affix.init(document.querySelector('.post-toc-widget'), Zoe.utils.getAffixParam());
});
